...
export class ModalTemplates {
    ...
    // #region STRUCTURE DE BASE
    // ============================================================================
    static getBaseHTML(currentLang: string, currentMode: string): string {
        return /*html*/ `
    ...
    // #region CONTENU DU POKÉMON
    // ============================================================================
    static getPokemonContentHTML(pokemon: Pokemon, mode: ImageMode, lang: Language, evoChain: EvolutionNode | null): string {
        return /*html*/ `
            <div class="modal-layout">
                ${this.buildLeftColumnHTML(pokemon, mode, lang)}
                ${this.buildRightColumnHTML(pokemon, mode, lang, evoChain)}
            </div>
        `;
    }

    private static buildLeftColumnHTML(pokemon: Pokemon, mode: ImageMode, lang: Language): string {
        return /*html*/ `
            <div class="modal-visual">
                ${this.buildPolaroidHTML(pokemon, mode, lang)}
                ${this.buildMetaGridHTML(pokemon)}
                ${this.buildEfficaciesContainerHTML()}
            </div>
        `;
    }

    private static buildRightColumnHTML(pokemon: Pokemon, mode: ImageMode, lang: Language, evoChain: EvolutionNode | null): string {
        ...
        return /*html*/ `
            <div class="modal-infos" style="margin-top: 1rem;">
                ...
                <div class="modal-types" style="display:flex; align-items:center; flex-wrap:wrap; gap:10px; margin-top:10px;">
                    ${this.buildTypeBadgesHTML(pokemon.types)}
                    ${this.buildCryButtonHTML()}
                </div>
                
                <div class="flavor-text" style="margin-top:20px;">"${desc}"</div>
                ...
                <div class="modal-evolutions">
                    ...
                    <div id="modal-evo-content">
                        ${this.buildEvolutionContainerHTML(evoChain, mode)}
                    </div>
                </div>
            </div>
        `;
    ...
    // #region GÉNÉRATEURS DE COMPOSANTS
    // ============================================================================
    private static buildPolaroidHTML(pokemon: Pokemon, mode: ImageMode, lang: Language): string {
        ...
        return /*html*/ `
            <div class="polaroid-frame sway-active" style="width: 240px; min-height: 280px; padding: 10px; display: flex; flex-direction: column; align-items: center; background: white; ${shadowStyle}">
                ...
                <div class="polaroid-id" style="margin-top: 10px; font-weight:bold; color:#555;">
    ...
    private static buildMetaGridHTML(pokemon: Pokemon): string {
    ...
    private static buildEfficaciesContainerHTML(): string {
        return /*html*/ `
    ...
    private static buildTypeBadgesHTML(types: string[]): string {
        return types.map(t => `<span class="type-badge type-${t}">${I18n.translateType(t)}</span>`).join('');
    }

    private static buildCryButtonHTML(): string {
        return /*html*/ `
    ...
    private static buildEvolutionContainerHTML(evoChain: EvolutionNode | null, mode: ImageMode): string {
        if (!evoChain) {
            return `<div class="evo-loading" style="min-height: 100px; display: flex; align-items: center; justify-content: center;">${I18n.t('loading')}...</div>`;
        }
        return /*html*/ `
            <div class="evo-tree-container" style="display:flex; justify-content:center; overflow-x:auto; padding:10px;">
                ${this.buildEvolutionHTML(evoChain, mode)}
            </div>
        `;
    }
    // #endregion

    // #region LOGIQUE STATS
    // ============================================================================
    static generateStatsHTML(stats: Stats): string {
        if (!stats) return '';
        
        return Object.entries(stats).map(([key, value]) => {
            const val = Number(value);
            const percent = Math.min(100, (val / 255) * 100);
    ...
    // #region LOGIQUE ÉVOLUTION
    // ============================================================================
    static buildEvolutionHTML(node: EvolutionNode, mode: ImageMode): string {
    ...
    // #region LOGIQUE TYPES & EFFICACITÉ
    // ============================================================================
    static getEfficaciesHTML(types: string[]): { weak: string, resist: string } {
        const effectiveness = TypeCalculator.getWeaknesses(types);

        return {
            weak: this.buildEfficacyBadges(effectiveness.weak),
            resist: this.buildEfficacyBadges([...effectiveness.immune, ...effectiveness.resist])
        };
    }

    private static buildEfficacyBadges(list: { type: string, value: number }[]): string {
